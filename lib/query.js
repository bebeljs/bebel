...
module.exports = class {
  ...
  setResponse (result) {
    ...
    if ({}.hasOwnProperty.call(result, 'body')) {
      this.setBody(result.body)
      changeResponse = true
    }
    if ({}.hasOwnProperty.call(result, 'info')) {
      this.setInfo(result.info)
      changeResponse = true
    }
    if ({}.hasOwnProperty.call(result, 'code')) {
  ...
  execCommand (application) {
    ...
    if (typeof func !== 'undefined') {
      if (application.$method[this.command] === 'command') {
        ...
        // private command
        this.res.send({ code: 500, info: `Command ${this.command} is private` })
      }
    } else {
      // undefined command
      this.res.send({ code: 500, info: `Command ${this.command} is not defined` })
    }
