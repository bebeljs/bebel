...
module.exports = class {
  ...
  registry (name, method, value) {
    ...
    try {
      if (method === 'instance') {
        let ObjectToInstance = (typeof value === 'string') ? require(value) : value
        Application.prototype[name] = new ObjectToInstance(this.application)
      }
      else {
        let resource = (typeof value === 'string') ? require(value) : value
  ...
  listen (port = 8000, cert = false) {
    ...
    try {
      ...
      this.application.$eventEmitter.emit('startServer', this.application)
    } catch (e) {
