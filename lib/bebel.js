...
module.exports = class {
  ...
  /**
   * @public
   * @param {Number | Object} listenOptions options for the http server listen
   * @param {Object} cert private key and certificate for https
   * @return this
   */

  listen (listenOptions = 8000, cert = false) {
    if (!this.started) {
      this.start()
    }
    ...
    try {
      if (typeof cert.key === 'string' && typeof cert.cert === 'string') {
        ...
        }, express).listen(listenOptions)
      } else {
        this.server = http.createServer(express).listen(listenOptions)
      }
